...
class CheckNewBulletinsUseCase @Inject constructor(
    ...
    suspend operator fun invoke(config: MailConfig): Result<CheckResult> = runCatching {
        ...
        if (currentDay < 16) {
            return Result.success(CheckResult.Skipped("Hors fenêtre temporelle (avant le 16)"))
        }
        ...
        if (hasNormal) {
            if (currentMonth == 12) {
                ...
                if (hasGratification) {
                    return Result.success(CheckResult.Stopped("Décembre : normal + gratification déjà détectés"))
                }
            } else {
                return Result.success(CheckResult.Stopped("Bulletin de $currentMonth/$currentYear déjà détecté"))
            }
        }
        ...
        if (newBulletins.isEmpty()) {
            return Result.success(CheckResult.NoResults)
        }
